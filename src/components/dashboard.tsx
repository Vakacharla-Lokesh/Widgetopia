...
const AVAILABLE_WIDGETS = [
  ...
  },
  {
    ...
    defaultLayout: { w: 2, h: 4, minW: 2, minH: 4 },
    preview: <Pomodoro widgetId="preview" />,
  },
  {
    type: "notes",
    name: "Notes",
...
export default function Dashboard() {
  ...
  const onLayoutChange = (
    ...
    if (
      ...
      JSON.stringify(widgets.map(({ i, x, y, w, h }) => ({ i, x, y, w, h })))
    ) {
  ...
  const removeWidget = (id: string) => {
    ...
    if (id.startsWith("pomodoro-")) {
      localStorage.removeItem(`pomodoro-work-${id}`);
      localStorage.removeItem(`pomodoro-break-${id}`);
    }
  };
  ...
  const renderWidget = (widget: WidgetItem) => {
    ...
    let content = <Component widgetId={widget.i} />;
    // Special case for calendar to avoid inner scrollbars and layout issues
  ...
  return (
    <>
      ...
      >{`
        body {
          background-image: ${theme.backgroundUrl
                ...
                <div className="grid gap-4 py-4 pr-4">
                  {AVAILABLE_WIDGETS.map((w) => (
                    ...
                    >
                      ...
                      <CardContent className="p-4 bg-muted/40 min-h-[150px] flex items-center justify-center">
